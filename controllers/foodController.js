...
async function purchaseFood(req, res) {
  try {
    const { id } = req.params;
    const { foodId, foodName, price, quantity, buyerName, buyerEmail } =
      req.body;
...
async function getTopSellingFoods(req, res) {
  try {
    ...
    res
      .status(500)
      .json({ message: "Failed to retrieve top-selling foods", error });
  }
}
  ...
  try {
    ...
    if (!updatedFood.value) {
      return res
        ...
        .json({
          message:
            "Food not found or you do not have permission to update this food",
        });
    }
